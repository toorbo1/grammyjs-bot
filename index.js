...
function isBotActive() {
    ...
    return (mskHours >= 12 && mskHours < 16) || (mskHours >= 17 && mskHours < 20); // Бот работает с 10:00 до 14:00 и с 15:00 до 21:00
}

// Обработка команды /start
bot.onText(/\/start/, (msg) => {
    if (!isBotActive()) {
        bot.sendMessage(msg.chat.id, 'Бот доступен для работы с 12:00 до 20:00 по мск. Обратите внимание, что с 16:00 до 17:00 у него обеденный перерыв. \n'+'Просим прощение от всей нашей команды за ваше ожидание\n');
        return;
    }
    ...
    const welcomeMessage = ('Приветствую вас\\!\n' +
        ...
        '\n' +
        'Время приема номеров\\: с 12\\:00 до 20\\:00 по московскому времени \\(MCK\\)\\.\n' +
        'С 16:00 до 17:00 обед \n'+
...
bot.on('message', (msg) => {
    ...
    if (!isBotActive()) {
        bot.sendMessage(chatId, 'Бот доступен для работы с 12:00 до 20:00 по мск. Обратите внимание, что с 16:00 до 17:00 у него обеденный перерыв. \n'+'Просим прощение от всей нашей команды за ваше ожидание\n');
...
bot.on('callback_query', (query) => {
    ...
    if (!isBotActive()) {
        bot.sendMessage(chatId, 'Бот не работает в данный момент.\n' +'Бот доступен для работы с 12:00 до 20:00 по мск. Обратите внимание, что с 16:00 до 17:00 у него обеденный перерыв. \n'+'Просим прощение от всей нашей команды за ваше ожидание\n');
